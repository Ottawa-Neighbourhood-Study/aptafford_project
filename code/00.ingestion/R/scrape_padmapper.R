...
#
# Code to collect daily rental unit data from Padmapper and push it to
# PostgreSQL server.
...
logdriver::add_log(level = "info", event = "Padmapper: Begin scraping", username = username)

newdata <- tryCatch(aptafford::padmapper_scrape(),
                    error = function(e) e)

if ("error" %in% colnames(newdata)){
  logdriver::add_log(level = "critical", event = "Padmapper: Scraping failed", username = username)
  return (0)
}

logdriver::add_log( event = "Padmapper: Scraping complete",
...
if ("error" %in% class(table_write)){
  logdriver::add_log(level = "critical", event = "Padmapper: Failed to write data to database.", username = username)
} else {
  logdriver::add_log(level = "info", event = "Padmapper: Success writing data to database.", username = username)
}
