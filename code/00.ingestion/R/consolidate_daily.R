...
rentalsca_clean <- rn %>%
  ...
  dplyr::select(title, property_type, bedrooms = beds, rent, address = address1, lat, lon = lng, unique_id, url) %>%
...
realtorca_clean <- re %>%
  ...
  select(title = public_remarks, property_type, bedrooms = building_bedrooms,
         ...
         lat = property_address_latitude, lon = property_address_longitude, unique_id, url) %>%
...
padmapper_clean <- pm %>%
  mutate(unique_id = sprintf("pm-%s-%s", building_id, listing_id)) %>%
  select(title = building_name, bedrooms, bathrooms = bathrooms, rent = price,
         address = address, lat = lat, lon = lng, unique_id, url) %>%
  mutate(title = strtrim(title, width = 100),
         source = "padmapper") %>%
  add_nad_sf() %>%
  dplyr::mutate(dplyr::across(-"geometry", as.character))
...
kijiji_clean <- kj %>%
  ...
  select(title, bedrooms, bathrooms, property_type = unit_type,
         rent = price, address = address, lat = lat, lon = lon, unique_id, url) %>%
